...
#[doc = "Field `TRACEID` reader - Trace ID field. Sets the trace ID value for instruction trace. Bit\\[0\\]
must be zero if data trace is enabled. If data trace is enabled then a trace unit sets the trace ID for data trace, to TRACEID+1."]
pub type TraceidR = crate::FieldReader;
#[doc = "Field `TRACEID` writer - Trace ID field. Sets the trace ID value for instruction trace. Bit\\[0\\]
must be zero if data trace is enabled. If data trace is enabled then a trace unit sets the trace ID for data trace, to TRACEID+1."]
pub type TraceidW<'a, REG> = crate::FieldWriter<'a, REG, 7>;
impl R {
    #[doc = "Bits 0:6 - Trace ID field. Sets the trace ID value for instruction trace. Bit\\[0\\]
must be zero if data trace is enabled. If data trace is enabled then a trace unit sets the trace ID for data trace, to TRACEID+1."]
    #[inline(always)]
...
must be zero if data trace is enabled. If data trace is enabled then a trace unit sets the trace ID for data trace, to TRACEID+1."]
    #[inline(always)]
    #[must_use]
    pub fn traceid(&mut self) -> TraceidW<TrctraceidrSpec> {
        TraceidW::new(self, 0)
    }
}
